...
class Pawn(AbstractPawn):
    ...
    def is_valid_move(self, board, pos1: str, pos2: str) -> bool:
        ahead: int = 1 if self.color == PawnColor.BLACK else -1
        ...
        if y1 == (1 if self.color == PawnColor.BLACK else 6) and x1 == x2 and y2 - y1 == 2 * ahead and board.get_pawn(pos2) is None and board.get_pawn(f"{pos1[0]}{int(pos1[1]) + ahead}") is None:
            return True
